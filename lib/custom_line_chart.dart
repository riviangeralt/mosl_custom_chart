...
/// Controller for MoCustomLineChart to programmatically control chart interactions
class MoCustomLineChartController {
  _MoCustomLineChartState? _state;
  ...
  void _attach(_MoCustomLineChartState state) {
    _state = state;
  }
...
/// Style configuration for line appearance
class LineStyle {
  final Color color;
...
class MoCustomLineChart<T> extends StatefulWidget {
  final List<T> data;
  ...
  final MoCustomLineChartController? controller;
  final num? minY;
  final num? maxY;
  final TooltipStyle? tooltipStyle;
  final LineStyle? lineStyle;

  const MoCustomLineChart({
    super.key,
    required this.data,
  ...
  @override
  State<MoCustomLineChart<T>> createState() => _MoCustomLineChartState<T>();
}

class _MoCustomLineChartState<T> extends State<MoCustomLineChart<T>> {
  int? _selectedPoint;
  ...
  @override
  void didUpdateWidget(MoCustomLineChart<T> oldWidget) {
  ...
  double _calculateLeftMargin() {
    ...
    if (minY < 0 && maxY > 0) {
      ...
      final negativeLabels =
          yLabels.where((label) => label < 0).toList()
  ...
  void _updateSelection(
    ...
    final double fixedPointSpacing =
        widget.pointSpacing ??
        (chartWidth / (widget.data.length > 1 ? widget.data.length - 1 : 1));
    ...
    final double sepY =
        topMargin + topPointGap + (chartHeight * (maxY - 0) / yRange);
    ...
    for (int i = 0; i < widget.data.length; i++) {
      ...
      pointY = pointY.clamp(
        topMargin + topPointGap,
        topMargin + topPointGap + chartHeight,
      );
      ...
      if (isTap) {
        isHit =
            (local.dx - pointX).abs() <= tapBuffer &&
            (local.dy - pointY).abs() <= tapBuffer;
      } else {
        isHit =
            (local.dx - pointX).abs() <= tapBuffer &&
...
class LineChartPainter<T> extends CustomPainter {
  final List<T> data;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double fixedPointSpacing =
        pointSpacing ?? (chartWidth / (data.length > 1 ? data.length - 1 : 1));
    const double pointsAreaPadding = 10.0;
    final double adjustedChartWidth = chartWidth - (pointsAreaPadding * 2);
    ...
    final double sepY =
        topMargin + topPointGap + (chartHeight * (chartMaxY - 0) / yRange);

    for (final yValue in yLabels) {
      final yPos =
          topMargin +
          topPointGap +
          (chartHeight * (chartMaxY - yValue) / yRange);

      if (yPos >= topMargin + topPointGap &&
          yPos <= topMargin + topPointGap + chartHeight) {
        TextSpan textSpan;
        ...
        textPainter.paint(canvas, Offset(1, yPos - textPainter.height / 2));
      }
    }
      ...
      for (int i = 0; i < data.length; i += stepSize) {
        ...
        textPainter.paint(
          ...
          Offset(
            ...
            topMargin + topPointGap + chartHeight + bottomSpacing + 4,
          ),
        );
      }
    }
    ...
    for (int i = 0; i < data.length; i++) {
      ...
      final y =
          value == null ? sepY : sepY - ((value - 0) / yRange) * chartHeight;
      ...
      if (value != null) {
        final pointPaint =
            Paint()
              ..color =
                  value >= 0 ? const Color(0xFF13861D) : const Color(0xFFDF130C)
    ...
    if (selectedPoint != null) {
      ...
      final actualTooltipWidth =
          contentWidth < 100.0 ? 100.0 : contentWidth.clamp(100.0, 200.0);
      ...
      // Calculate actualArrowXOffset to align arrow with pointX
      ...
      if (value != null) {
        ...
        final shadowPaint =
            Paint()
              ...
              ..style = PaintingStyle.fill;
        canvas.drawCircle(pointCenter, lineStyle.pointRadius + 2, shadowPaint);
        final dotPaint =
            Paint()
              ..color = Colors.white
              ..style = PaintingStyle.fill;
        canvas.drawCircle(pointCenter, lineStyle.pointRadius, dotPaint);
        final borderPaint =
            Paint()
              ..color = Colors.black
              ..style = PaintingStyle.stroke
              ..strokeWidth = 0.4;
        canvas.drawCircle(pointCenter, lineStyle.pointRadius, borderPaint);
      }
    }
  }
  ...
  @override
  bool shouldRepaint(covariant LineChartPainter<T> oldDelegate) {
